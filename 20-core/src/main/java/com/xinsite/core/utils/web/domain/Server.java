package com.xinsite.core.utils.web.domain;
...
public class Server {
     ...
     * 刷新服务器信息
     */
    public static void refreshServerInfo(boolean reload) {
        if (server == null) {
            server = new Server();
            server.copyTo(true);
        } else {
            server.copyTo(reload);
        }
    }
    ...
    public void copyTo(boolean reload) {
        long past_seconds = DateUtils.pastSeconds(lastLoadTime);
        if (past_seconds >= 30 || reload) {  //30秒刷新一次
            try {
    ...
    private void setSysFiles(OperatingSystem os) {
        ...
        if (sysFiles.size() > 0 && fsArray.length > 0) sysFiles.clear();
