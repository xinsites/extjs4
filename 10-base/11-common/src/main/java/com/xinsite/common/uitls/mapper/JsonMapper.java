package com.xinsite.common.uitls.mapper;
...
//		final String dataFormat = Global.getProperty("json.mappings.dataFormat");
...
//        }));
     ...
     */
    @SuppressWarnings("unchecked")
    public static <T> List<T> fromJsonForMapList(JSONArray array, Class<?> clazz) {
        List result = new ArrayList();
        if (array != null) {
            for (int i = 0; i < array.length(); i++) {
                JSONObject jsonObject = array.getJSONObject(i);
                try {
                    result.add(fromJson(jsonObject.toString(), clazz));
                } catch (Exception e) {
