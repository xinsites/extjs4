...
import com.xinsite.common.uitls.callback.MethodCallback;
...
import com.xinsite.common.uitls.reflect.ReflectUtils;
...
public class ListUtils extends org.apache.commons.collections.ListUtils {
     ...
     * @author www.xinsite.vip
     */
    public static <T> List<T> listOrderBy(List<T> list, String orderBy, String fieldType) {
                ...
                Collections.sort(list, new Comparator<T>() {
                    public int compare(T o1, T o2) {
                        ...
                        if (o1 instanceof Map) {
                            s1 = ObjectUtils.objToStr(((Map) o1).get(ss[0]));
                            s2 = ObjectUtils.objToStr(((Map) o2).get(ss[0]));
                        } else if (o1 instanceof JsonObject) {
                            s1 = GsonUtils.tryParse((JsonObject) o1, ss[0]);
                            s2 = GsonUtils.tryParse((JsonObject) o2, ss[0]);
                        } else {
                            s1 = ObjectUtils.objToStr(ReflectUtils.invokeGetter(o1, ss[0]));
                            s2 = ObjectUtils.objToStr(ReflectUtils.invokeGetter(o2, ss[0]));
                        }
                        if ("int".equalsIgnoreCase(fieldType)) {
                            int int1 = ValueUtils.tryParse(s1, 0);
                            int int2 = ValueUtils.tryParse(s2, 0);
                            if ("asc".equals(ss[1])) {
                                return int1 - int2;
                            } else {
                                return int2 - int1;
