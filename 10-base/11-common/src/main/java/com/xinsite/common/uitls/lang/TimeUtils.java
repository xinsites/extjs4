...
public class TimeUtils {
    ...
    /**
     * 到给定的时间，还剩余多少时间，xx秒，xx分钟，xx小时、xx天前，大于99天的显示日期
     */
    public static String surplusTimeToDate(String dateTime) {
        return surplusTimeToDate(DateUtils.parseDate(dateTime));
    }

    /**
     * 到给定的时间，还剩余多少时间，xx秒，xx分钟，xx小时、xx天前，大于99天的显示日期
     */
    public static String surplusTimeToDate(Date dateTime) {
        if (dateTime == null) return "";
        String interval = null;
        long time = dateTime.getTime() - System.currentTimeMillis();
        if (time < 10) return "失效";
